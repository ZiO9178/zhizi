...
local Toggle = Tab:Toggle({
    ...
    Callback = function(state)
        if workspace:FindFirstChild("KillerESP") then
            workspace.KillerESP:Destroy()
        end
        
        if state then
            local ESPFolder = Instance.new("Folder")
            ESPFolder.Name = "KillerESP"
            ...
            local function updateESP()
                ...
                for _, player in ipairs(game.Players:GetPlayers()) do
                    ...
                    if character and player.Team == KillerTeam then
                        ...
                        if humanoid and rootPart then
                            ...
                            box.Position = Vector3.new(0, 3, 0)
                            ...
                            healthFill.BorderColor3 = Color3.new(0, 0, 0)

                            humanoid.HealthChanged:Connect(function(health)
                                local ratio = math.clamp(health / humanoid.MaxHealth, 0, 1)
                                healthFill.Size = UDim2.new(ratio, 0, 1, 0)
                                healthFill.BackgroundColor3 = ratio > 0.5 and Color3.new(0, 1, 0) or Color3.new(1, 0.5, 0)
                            end)
...
local Toggle = Tab:Toggle({
    ...
    Locked = false,
    Callback = function(state)
        if workspace:FindFirstChild("SurvivorESP") then
            workspace.SurvivorESP:Destroy()
        end
        
        if state then
            local ESPFolder = Instance.new("Folder")
            ESPFolder.Name = "SurvivorESP"
            ESPFolder.Parent = workspace
            ...
            local function updateSurvivorESP()
                ...
                for _, player in ipairs(game.Players:GetPlayers()) do
                    local character = player.Character
                    if character and player.Team == SurvivorTeam then
                        local humanoid = character:FindFirstChildOfClass("Humanoid")
                        local rootPart = character:FindFirstChild("HumanoidRootPart")
                        if humanoid and rootPart and humanoid.Health > 0 then
                            ...
                            box.Name = "ESPBox"
                            box.Adornee = rootPart
                            box.Size = Vector3.new(3.8, 5.8, 1.8)
                            box.Position = Vector3.new(0, 2.9, 0)
                            box.Color3 = Color3.new(0, 0.8, 1)
                            box.Transparency = 0.3
                            box.ZIndex = 5
                            box.Parent = ESPFolder
                            ...
                            nameText.Parent = nameLabel
                            nameText.Size = UDim2.new(1, 0, 0.5, 0)
                            nameText.Text = player.Name .. " [幸存者]"
                            nameText.TextColor3 = Color3.new(0, 0.8, 1)
                            nameText.BackgroundTransparency = 1
                            nameText.TextScaled = true
                            nameLabel.Parent = ESPFolder
                            ...
                            updateHealthColor(humanoid.Health)
                            humanoid.HealthChanged:Connect(updateHealthColor)
                            healthBar.Parent = ESPFolder
                        end
                    end
                end
            ...
            updateSurvivorESP()
            game.Players.PlayerAdded:Connect(function(player)
                player.CharacterAdded:Connect(updateSurvivorESP)
            end)
        end
    end
})