...
local Button = Tab:Button({
    ...
    Locked = false,
    Callback = function()
        local targetPart = workspace:FindFirstChild("RoadServiceSpawn", true)
        if targetPart and targetPart:IsA("BasePart") then
            tpTo(targetPart.CFrame + Vector3.new(0, 3, 0))
            return
        end

        tpTo(CFrame.new(0, 10, 0))
    end
})
...
local Button = Tab:Button({
    ...
    Locked = false,
    Callback = function()
        local folder = workspace.Gameplay:FindFirstChild("Fire Departament")
        if not folder then return end

        local target = folder:FindFirstChild("Entrance", true)
            or folder:FindFirstChild("Spawn", true)
            or folder:FindFirstChildWhichIsA("BasePart", true)

        if target and target:IsA("BasePart") then
            tpTo(target.CFrame + Vector3.new(0, 3, 0))
        end

        local r = folder:FindFirstChild("_TeamChangeWithEntrance")
...
local Button = Tab:Button({
    ...
    Locked = false,
    Callback = function()
        local hospital = workspace.Gameplay:FindFirstChild("Hospital")
        if not hospital then return end

        local target = hospital:FindFirstChild("Entrance", true)
            or hospital:FindFirstChild("Spawn", true)
            or hospital:FindFirstChild("Door", true)
            or hospital:FindFirstChildWhichIsA("BasePart", true)

        if target and target:IsA("BasePart") then
            tpTo(target.CFrame + Vector3.new(0, 3, 0))
        end

        local r = hospital:FindFirstChild("_TeamChangeWithEntrance")
        if r and r:IsA("RemoteEvent") then
            r:FireServer()
        elseif r and r:IsA("RemoteFunction") then
            pcall(function() r:InvokeServer() end)
...
local Button = Tab:Button({
    ...
    Locked = false,
    Callback = function()
        local police = workspace.Gameplay:FindFirstChild("Police Departament")
        if not police then return end

        local target = police:FindFirstChild("Entrance", true)
            or police:FindFirstChild("Spawn", true)
            or police:FindFirstChild("Door", true)
            or police:FindFirstChildWhichIsA("BasePart", true)

        if target and target:IsA("BasePart") then
            tpTo(target.CFrame + Vector3.new(0, 3, 0))
        end

        local r = police:FindFirstChild("_TeamChangeWithEntrance")
        if r and r:IsA("RemoteEvent") then
            r:FireServer()
        elseif r and r:IsA("RemoteFunction") then
            pcall(function() r:InvokeServer() end)
