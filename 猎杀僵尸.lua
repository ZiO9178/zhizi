            ...
            spawn(function()
                while state do
                    for _, zombie in pairs(workspace.Zombies:GetChildren()) do
                        if zombie:IsA("Model") and zombie:FindFirstChild("HumanoidRootPart") and zombie:FindFirstChild("Humanoid") and zombie.Humanoid.Health > 0 then
                            local targetPos = zombie.HumanoidRootPart.Position
                            local cameraPart = workspace.Camera.Part
                            local playerPos = cameraPart.Position
