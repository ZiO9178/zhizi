...
local Toggle = Tab:Toggle({
    ...
    Callback = function(state)
        ...
        while _G.autoCollect do
            local items = {
                workspace.Map.Util.Items.AmmoARBasic,
                workspace.Map.Util.Items.AmmoPistolBasic,
                workspace.Map.Util.Items.AmmoShotgunBasic
            }

            for _, item in ipairs(items) do
                if item and item.Parent then
                    game:GetService("ReplicatedStorage").Remotes.RequestPickupItem:FireServer(item)
                end
            end
...
local Toggle = Tab:Toggle({
    ...
    Callback = function(state)
        ...
        pick(workspace.Map.Util.Items.Cake)
        pick(workspace.Map.Util.Items.Cola)
        pick(workspace.Map.Util.Items.Burger)
    end
})
...
local Toggle = Tab:Toggle({
        ...
        Callback = function(state)
        local args = {
    [1] = workspace.Map.Util.Items.Bandage
}

game:GetService("ReplicatedStorage").Remotes.RequestPickupItem:FireServer(unpack(args))     
        end
    })
...
local Toggle = Tab:Toggle({
    ...
    Callback = function(state)
        if state then
            game:GetService("ReplicatedStorage").Remotes.RequestPickupItem:FireServer(workspace.Map.Util.Items.BasicFlashlight_Standard)

            game:GetService("ReplicatedStorage").Remotes.RequestPickupItem:FireServer(workspace.Map.Util.Items.BasicFlashlight_Big)
        end
    end
})
...
local Toggle = Tab:Toggle({
    ...
    Callback = function(state)
        if state then
            local args1 = {
                [1] = workspace.Map.Util.Items.Plank
            }
            game:GetService("ReplicatedStorage").Remotes.RequestPickupItem:FireServer(unpack(args1))

            local args2 = {
                [1] = workspace.Map.Util.Items.Katana
            }
            game:GetService("ReplicatedStorage").Remotes.RequestPickupItem:FireServer(unpack(args2))
        else
        end
    end
})
...
local Toggle = Tab:Toggle({
    ...
    Callback = function(state)
        ...
        local remote = rs.Remotes.RequestPickupItem

        remote:FireServer(workspace.Map.Util.Items.Bandage)
        remote:FireServer(workspace.Map.Util.Items.Medkit)
    end
})
...
local Toggle = Tab:Toggle({
    ...
    Locked = false,
    Callback = function(state)
        local characterData = workspace.r4dhzm91.CharacterData
        if state then
            characterData.MaxEnergy = math.huge
            characterData.Energy = math.huge
        else
            characterData.MaxEnergy = 100
            characterData.Energy = 100
        end
    end
})
    ...
    Callback = function(state)
        ...
        if state then
            heartbeatConnection = RunService.Heartbeat:Connect(function()
                local characterData = workspace:FindFirstChild("r4dhzm91") and workspace.r4dhzm91:FindFirstChild("CharacterData")
                if characterData and characterData:FindFirstChild("MaxStamina") then
