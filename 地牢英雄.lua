...
local Toggle = Tab:Toggle({
    ...
    Callback = function(state)
        if _G.AutoAttackLoop then
            task.cancel(_G.AutoAttackLoop)
            _G.AutoAttackLoop = nil
        end
        
        if state then
            _G.AutoAttackLoop = task.spawn(function()
                local CombatRemote = game:GetService("ReplicatedStorage").Systems.Combat.PlayerAttack
