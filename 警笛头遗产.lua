...
local Toggle = Tab:Toggle({
     Title = "自动收集战利品",
     Desc = "启用后自动收集场景中所有可拾取物品",
     Locked = false,
     Callback = function(state)
         if state then
 -- 创建循环收集的协程并保存引用 
             Toggle.LootCollectionRoutine = coroutine.create(function()
                 while true do
 -- 1. 获取所有战利品
                     local lootItems = workspace.Loot:GetDescendants()
                     local collectedCount = 0
                     
 -- 2. 过滤有效物品（可根据实际需求调整条件）
                     for _, item in ipairs(lootItems) do
                         if item:IsA("BasePart") and item:GetAttribute("CanPickup") ~= false then 
 -- 3. 模拟收集逻辑（需根据游戏实际API调整）
                             firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, item, 0) -- 开始接触
                             firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, item, 1) -- 结束接触
                             collectedCount = collectedCount + 1
                             task.wait(0.1) -- 防止服务器检测为作弊 
                         end
                     end 
                     
 -- 4. 输出收集报告 
                     print(string.format("[自动收集] 本轮收集完成，共获得 %d 件物品", collectedCount))
                     task.wait(1) -- 收集间隔
                 end 
             end)
             
 -- 启动协程
             coroutine.resume(Toggle.LootCollectionRoutine)
         else
 -- 关闭时终止协程 
             if Toggle.LootCollectionRoutine then
                 coroutine.close(Toggle.LootCollectionRoutine)
             end
         end 
     end
 })