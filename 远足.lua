...
local Tabs = {
    Main = Window:Section({ Title = "物资功能", Opened = true })
}

local TabHandles = {
    Elements = Tabs.Main:Tab({ Title = "武器", Icon = "layout-grid", Desc = "武器" })
}

TabHandles.Elements:Button({
    Title = "Mine Detonator",
    Icon = "bell",   
    Callback = function()
    local args = {
    [1] = "Mine Detonator",
    [2] = workspace.Objects:FindFirstChild("Mine Detonator")
}

game:GetService("ReplicatedStorage").Remotes.PickupItem:FireServer(unpack(args))
    end
})
...
TabHandles.Elements:Button({
    ...
    local args = {
    [1] = "Spiked Club",
    [2] = workspace.Objects:FindFirstChild("Spiked Club")
}
