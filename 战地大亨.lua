...
local Toggle = Tab:Toggle({
    ...
    Callback = function(state)
        if state then
            ...
            originalFireServer = hookfunction(game:GetService("ReplicatedStorage").ACS_Engine.Events.ServerBullet.FireServer, function(self, ...)
                local args = {...}
                
                if type(args[1]) == "table" and args[1].weaponName then
                    if args[1].currentPenetrationCount then
                        args[1].currentPenetrationCount = 0
                    end
            ...
            if originalFireServer then
                restorefunction(game:GetService("ReplicatedStorage").ACS_Engine.Events.ServerBullet.FireServer)
