...
local Toggle = Tab:Toggle({
    ...
    Callback = function(state)
        if state then
            local eating = true
            while eating do
                game:GetService("ReplicatedStorage").Packages.Knit.Services.GrabService.RF.Munch:InvokeServer()
                wait(0)
            end
        else
            eating = false
